...
// 2. Define a target vector matching the expected dummy file AST hash vector
const scannerInitial = new Scanner(testDir);
// Just build the exact target vector for the target state string
const tempDummy = path.join(testDir, '_temp_target.md');
fs.writeFileSync(tempDummy, 'initial state\n\ndeterministic code assert pure\n\n');
const targetVector = scannerInitial.aggregateVectors(scannerInitial.scanDirectory());
fs.rmSync(tempDummy);
fs.writeFileSync(targetPath, JSON.stringify({ vector: Array.from(targetVector) }));
